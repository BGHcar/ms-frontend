...
export class ListComponent implements OnInit {
  ...
  servicios: Ejecucionservicio[];
  theServicio: Ejecucionservicio;
  ...
  delete(id: number) {
    Swal.fire({
      title: 'Eliminar la ejecucion de su servicio',
      ...
      if (result.isConfirmed) {
        this.service.delete(id).subscribe(data => {
          Swal.fire(
            'Eliminado!',
            'El la ejecucion del servicio ha sido eliminada correctamente',
            'success'
          )
          this.ngOnInit();
        });
